...
- 有显卡的情况下，或者想要执行一个模拟全流程，推荐完成“2.1”。
# 在Windows系统安装
...
# 在Linux系统安装
首先[安装一个Linux环境](getLinux.md)，再进行以下的步骤。有两种解决方案：
1. 某些发行商预编译的版本
   - Conda-forge社区编译的GROMACS版本最全最新。因此通过 [Conda](https://docs.anaconda.com/miniconda/) 包管理工具，直接搭建一个用于分子模拟虚拟环境是个不错的选择。
   - 此外，这个环境可以不仅仅包含GROMACS，还能安装其它模拟过程中需要用到的工具，因此非常推荐使用该方式。省去了编译中易报错的缺点，同时可以一步到位安装其它必要的工具。
2. 从源码编译安装（理论上效率最高，但也最复杂），有能力的可以尝试。
## 安装预编译版本
### 安装conda环境
打开Linux命令行，输入以下命令：
```bash
...
创建一个名为ms的环境，并安装对应的GROMACS版本：个人电脑安装nompi版，有nvidia显卡安装cuda版，没有dblprec表示单精度版（足够）。
```
...
```
如果你觉得上述过程还是麻烦，可以使用这里写好的文件：[ms_env.yml](/ms_env.yml)，然后运行一个命令安装，可以代替上面 1、2 两个步骤。  
